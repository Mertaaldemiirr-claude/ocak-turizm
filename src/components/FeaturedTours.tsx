    ...
    <section id="turlar" className="py-16 lg:py-20 bg-white">
      <div className="max-w-6xl mx-auto px-4">
          ...
          {tours.map((tour) => (
            ...
            >
              <div className="relative h-48 overflow-hidden bg-gray-200">
                {tour.imageUrl ? (
                  <Image src={tour.imageUrl} alt={tour.name} fill className="object-cover group-hover:scale-105 transition-transform duration-300" sizes="(max-width: 768px) 100vw, 33vw" />
                ) : (
                  <div className="w-full h-full flex items-center justify-center text-gray-400 text-sm">{t.noImage}</div>
                )}
              ...
              <div className="p-4">
                <h3 className="font-heading font-semibold text-primary text-base mb-1.5">{tour.name}</h3>
                  ...
                  <div className="flex items-center gap-2">
                    {(() => {
                      ...
                      return (
                        <>
                          {tour.oldPrice && (
                            <span className="text-gray-400 text-sm line-through">{sym}{tour.oldPrice.toLocaleString("tr-TR")}</span>
                          )}
                          <span className="font-heading font-bold text-gold text-lg">{sym}{tour.price?.toLocaleString("tr-TR")}</span>
                        </>
                      );
                    })()}
                  </div>
        ...
        <div className="text-center mt-8">
          <Link href={lp("/turlar")} className="text-primary hover:text-gold font-semibold text-sm transition-colors">
            {t.viewAll} &rarr;
          </Link>
        </div>
      </div>
